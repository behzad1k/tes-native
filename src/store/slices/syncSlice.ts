...
export const startSync = createAsyncThunk(
  ...
  async (_, { getState, dispatch, rejectWithValue }) => {
    try {
      ...
      const unsyncedSigns = state.signs.signs.filter((sign: any) =>
      ...
      // Process each unsynced sign
      for (const sign of unsyncedSigns) {
        dispatch(setCurrentOperation(`Syncing sign ${sign.signId || sign.localId}`));

        try {
          if (sign.isNew) {
            // CREATE new sign with images
            ...
            dispatch(updateAfterSync({
              localId: sign.localId!,
              serverId: response.data.id,
              imageUpdates: response.data.images.map((img: any, idx: number) => ({
            ...
            // UPDATE - Add new images to existing sign
            const formData = new FormData();
            ...
            // UPDATE sign data only
            const { localId, isNew, status, images, serverId, ...signData } = sign;
          ...
          continue; // Continue with next sign
        }
...
const syncSlice = createSlice({
  ...
  reducers: {
    ...
    updatePendingCounts: (state, action: PayloadAction<{
      creates: number;
      updates: number;
      deletes: number;
      images: number;
