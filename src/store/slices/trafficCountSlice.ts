  ...
  reducers: {
    ...
    addCountToWorkOrder: (
      ...
      if (wo) {
        ...
        // Each drag-drop = new count record
        wo.counts.push(action.payload.count);
        wo.isEdited = true;
        wo.isSynced = false;
        wo.syncStatus = "Not Synced";
        // Auto-transition from To Do -> In Progress
        if (wo.status === "To Do") {
    ...
    builder
      ...
      .addCase(fetchWorkOrders.fulfilled, (state, action) => {
        ...
        state.classifications =
          action.payload.classifications as TrafficCountClassification[];
      ...
      .addCase(syncTrafficCountData.fulfilled, (state, action) => {
        if (action.payload.workOrders?.length > 0) {
          state.workOrders =
            action.payload.workOrders as TrafficCountWorkOrder[];
        }
